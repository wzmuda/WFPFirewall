...
DEFINE_GUID(FIREWALL_ENGINE_CALLOUT_DATA_LIMIT_KEY, 0xee94ec3dL, 0x04d7, 0x44d0, 0xb7, 0x65, 0x38, 0x41, 0xad, 0xfa, 0x2f, 0x38);  // TODO make it common for kernel module and here
...
bool FirewallEngine::addFilterDataLimit(std::string host, uint32_t ip, uint32_t mask, uint64_t data_limit_bytes) {
    ...
    filter.displayData.name = const_cast<wchar_t*>(L"Wojtek's WFPFirewall data limit filter");
    filter.displayData.description = const_cast<wchar_t*>(L"Wojtek's WFPFirewall data limit filter");
